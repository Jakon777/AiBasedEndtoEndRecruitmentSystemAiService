...
def _skills_overlap(required: List[str], candidate_skills: List[str]):
    if not required:
        return 1.0, [], []
...
def build_job_text(job: Dict[str, Any]) -> str:
    ...
    skills = job.get("skillsRequired") or []
    if skills:
        parts.append("Required skills: " + ", ".join(map(str, skills)))

    if job.get("experienceRequired") is not None:
        parts.append(f"Minimum experience: {job['experienceRequired']} years")

    if job.get("jobType"):
        parts.append(f"Job type: {job['jobType']}")

    return "\n".join(parts)[:4000]  # ✅ limit size
...
def evaluate_shortlist(job: Dict[str, Any], resume_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        resume_text = (
            parsed.get("text_for_similarity")
            or parsed.get("text_preview")
            or ""
        ).strip()
        ...
        # ✅ FAST now (model already loaded)
        similarity = max(0.0, min(1.0, compute_similarity(job_text, resume_text)))

        skill_ratio, matched, missing = _skills_overlap(required, candidate_skills)

        combined = (
            similarity * 100.0
            if not required
            else (_SIMILARITY_WEIGHT * similarity + _SKILL_WEIGHT * skill_ratio) * 100.0
        )

        return {
            "shortlisted": combined >= _SHORTLIST_THRESHOLD,
